...
public class JedisScheduleAdapter extends BaseJedisAccessor implements ScheduleAdapter {
	...
	public void start(final WorkflowManager workflowManager) {
		final Runnable runnable = () -> {
			...
			if (leader.get()) {
				final String value = getFromRedis(jedis -> jedis.get(keyNamesCreator.getLeaderElectionKey()));
		...
		workflowManager.scheduledExecutorService().scheduleAtFixedRate(runnable, 0, 30, TimeUnit.SECONDS);
	}
